...
module.exports = (db) => {
  ...
  const addLike = function(value, resource_id, user_id) {
    let queryString = `
    INSERT INTO resource_reviews(liking, resource_id, user_id) VALUES($1, $2, $3)`

      return db.query(queryString, [value, resource_id, user_id])
  }
  ...
  router.post("/:resource_id/likes/new", (req, res) => {
    const like = req.params.value;
    const resource_id = req.params.resource_id;
    const user_id = req.session.userId;

    addLike(like, resource_id, user_id)
    .then(dbRes => res.json("OK"))
  });
